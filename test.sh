...
const cases = [
  ...
  ["Esc closes an open dropdown first",   M.composeEscape(true, true, false), "popup"],
  ["then warns about a draft",            M.composeEscape(false, true, false), "arm"],
  ["and cancels it the second time",      M.composeEscape(false, true, true), "cancel"],
  ["a blank form cancels at once",        M.composeEscape(false, false, false), "cancel"],
  ["cancelling goes back to the list",    JSON.stringify(M.cameFrom("mine", 0)), '{"mode":"mine","storyId":0}'],
  ...
  ["an empty search is the whole list", M.searchStories([{id: 1, name: "A"}, {id: 2, name: "B"}], refs, "  ").length, 2],
  ["a search finds a name, any case",   M.searchStories([{id: 1, name: "Alpha"}, {id: 2, name: "Beta"}], refs, "BET").map(s => s.name).join(","), "Beta"],
  ["or a number, with sc-",             M.searchStories([{id: 1234, name: "X"}, {id: 99, name: "Y"}], refs, "sc-1234").length, 1],
  ["or without it",                     M.searchStories([{id: 1234, name: "X"}, {id: 99, name: "Y"}], refs, "123").length, 1],
  ["or an owner",                       M.searchStories([{id: 1, name: "X", ownerIds: ["ada-uuid"]}, {id: 2, name: "Y"}], refs, "ada").length, 1],
  ["every word has to match",           M.searchStories([{id: 1, name: "Fix login", ownerIds: ["ada-uuid"]}, {id: 2, name: "Fix login"}], refs, "login ada").length, 1],
  ...
  ["an edit",                          M.panelTitle({mode: "mine", storyId: 1234, editingId: 1234}), "Edit sc-1234"],
  ["solving it",                       M.panelTitle({mode: "mine", storyId: 1234, solveReview: true}), "Solve sc-1234"],
  ["opening its pull request",         M.panelTitle({mode: "mine", storyId: 1234, prReview: true}), "Pull request for sc-1234"],
  ["the settings, even locked",        M.panelTitle({mode: "settings", locked: true}), "Settings"],
  ["locked asks for the token",        M.panelTitle({mode: "mine", locked: true}), "Connect to Shortcut"],
  ...
  ["the PR the header already shows is left out",
    M.linkChips(["https://github.com/a/b/pull/9/files", "https://figma.com/f"], "https://github.com/a/b/pull/9").map(c => c.label).join(","), "figma.com"],
  ["another PR is its own chip, by reference",
    M.linkChips(["https://github.com/a/b/pull/10"], "https://github.com/a/b/pull/9")[0].label, "a/b#10"],
  ["a GitHub issue says what it is",
    M.linkChips(["https://github.com/a/b/issues/3"], "")[0].label, "a/b issues 3"],
  ["a repository link is just the repository",
    M.linkChips(["https://github.com/a/b"], "")[0].label, "a/b"],
  ...
  ["the heading names it",
    M.storyRows([{heading: "Prioritized", commonState: "Prioritized",
                  stories: [{stateName: "Prioritized"}, {stateName: "Review"}]}])[0].title, "Prioritized"],
  ["a row in that state leaves it out",
    M.storyRows([{heading: "h", commonState: "Prioritized", stories: [{stateName: "Prioritized"}]}])[1].showState, false],
  ["a row in another state says it",
    M.storyRows([{heading: "h", commonState: "Prioritized", stories: [{stateName: "Review"}]}])[1].showState, true],
  ["with no shared state every row says its own",
    M.storyRows([{heading: "h", commonState: "", stories: [{stateName: "Review"}]}])[1].showState, true],
...
cases.push(
  ...
  ["each state in a kind gets its own heading",
    ...
    "In Development:B|Code Review:CA"],
  ...
  ["an unseen story is news",
    M.newlyAssigned([{id:1},{id:2}], ["1"], []).notify.map(s => s.id).join(","), "2"],
  ["and is remembered as told",
    M.newlyAssigned([{id:1},{id:2}], ["1"], []).notified.join(","), "2"],
  ["a story told about is not told again",
    M.newlyAssigned([{id:1},{id:2}], ["1"], ["2"]).notify.length, 0],
  ["a seen story is not news", M.newlyAssigned([{id:1}], ["1"], []).notify.length, 0],
  ["a story with no id is not news", M.newlyAssigned([{name:"x"}], [], []).notify.length, 0],
  ["one that left is forgotten", M.newlyAssigned([{id:1}], ["1"], ["1","9"]).notified.join(","), "1"],
  ["so it is news when it comes back",
    M.newlyAssigned([{id:1},{id:9}], ["1"], M.newlyAssigned([{id:1}], ["1"], ["1","9"]).notified)
      .notify.map(s => s.id).join(","), "9"],
  ["one story says its name",
    M.assignedNotices([{id:12, name:"Fix the thing"}], "p")[0].body, "sc-12 · Fix the thing"],
  ["and a click opens it",
    M.assignedNotices([{id:12, name:"x"}], "p")[0].argv,
    ["omarchy-shell","shell","summon","p",'{"mode":"mine","story":12}']],
  ["three are three notifications",
    M.assignedNotices([{id:1},{id:2},{id:3}], "p").length, 3],
  ["four are one",
    M.assignedNotices([{id:1},{id:2},{id:3},{id:4}], "p").map(n => n.summary), ["4 stories assigned to you"]],
  ["which opens the list",
    M.assignedNotices([{id:1},{id:2},{id:3},{id:4}], "p")[0].argv[4], '{"mode":"mine"}'],
  ["nothing new says nothing", M.assignedNotices([], "p").length, 0],
  ["a name is shown as written, not as markup",
    M.assignedNotices([{id:5, name:"Fix <Button> & <img src=x>"}], "p")[0].body,
    "sc-5 · Fix &lt;Button&gt; &amp; &lt;img src=x&gt;"],
  ["a notification stays until dismissed",
    M.noticeCommand(M.assignedNotices([{id:5, name:"x"}], "p")[0]).slice(0, 5),
    ["notify-send","-a","Shortcut Stories","-u","critical"]],
  ["and ends with what it says",
    M.noticeCommand(M.assignedNotices([{id:5, name:"x"}], "p")[0]).slice(-2), ["Assigned to you","sc-5 · x"]],
  ["a bug carries the bug glyph",
    M.assignedNotices([{id:5, storyType:"bug"}], "p")[0].glyph, M.storyGlyph("bug")],
  ["notifying is on by default", M.readSetting({}, "notifyAssigned"), true],
  ["and can be turned off", M.readSetting({notifyAssigned: "off"}, "notifyAssigned"), false],
  ...
  ["and the section it sat alone in goes with it",
    M.settingsPage(false).flat().map(x => x.title).includes("Shortcut"), false],
  ["but not from a linked checkout",
    M.settingsPage(true).flat().flatMap(x => x.rows).some(r => r.key === "demo"), true],
  ["linked shows it", M.showDevSettings(true, {}), true],
  ["a clone does not", M.showDevSettings(false, {}), false],
  ["unless it is on, so it can be turned off", M.showDevSettings(false, {demo: "on"}), true],
  ["and not once it is off again", M.showDevSettings(false, {demo: "off"}), false],
